  ...
  _createTimers() {
    ...
    for (let [interval, rules] of Object.entries(this.rulesByInterval)) {
      const handler = async () => {
        ...
        const readings = await this._readRuleSensors()

        for (let rule of rules) {
          if (!rule.matches(readings[rule.sensor])) {
            this.logger.info(
              'sensor reading %s does not match rule threshold of %s',
          ...
          this.logger.info('rule matched - set output %s to state %s', rule.action.output, rule.action.state)

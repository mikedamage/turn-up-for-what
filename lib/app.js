...
export default class App {
  ...
  _createTimers() {
    ...
    for (let [interval, rules] of Object.entries(this.rulesByInterval)) {
      const handler = async (firstRun = false) => {
        ...
        for (let rule of rules) {
          // Don't run rules with immediate: false if this is the very first run before their timer is actually triggered
          if (firstRun && !rule.immediate) continue
          
          if (!rule.matches(readings[rule.sensor])) {
            this.logger.debug(
              'sensor reading %s does not match rule threshold of %s %s',
            ...
            if (rule.resetWhenNegative) {
              this.logger.debug('resetting output to default value')
